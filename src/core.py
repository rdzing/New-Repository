...
class ImageInstanceOps:
    ...
    def read_omr_response(self, template, image, name, save_dir=None,preserve_size=False):
        config = self.tuning_config
        auto_align = config.alignment_params.auto_align
        try:
            img = image.copy()
            h, w = img.shape[:2]
            template_w, template_h = template.page_dimensions
            if not preserve_size and (w != template_w or h != template_h):
                img = ImageUtils.resize_util(img, template_w, template_h)
            # origDim = img.shape[:2]
            if img.max() > img.min():
            ...
            if config.outputs.show_image_level >= 2:
                initial_align = self.draw_template_layout(img, template, shifted=False, preserve_size=True)
                final_align = self.draw_template_layout(
                    img, template, shifted=True, draw_qvals=True, preserve_size=True
                )
    ...
    @staticmethod
    def draw_template_layout(img, template, shifted=True, draw_qvals=False, border=-1, preserve_size=False):
        # First ensure input image has correct dimensions
        h, w = img.shape[:2]
        template_w, template_h = template.page_dimensions
        if not preserve_size and (w != template_w or h != template_h):
            img = ImageUtils.resize_util(img, template_w, template_h)
